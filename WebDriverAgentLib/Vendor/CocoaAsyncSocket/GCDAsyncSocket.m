...
// Note: CFStream APIs are still used for TLS support and are part of CoreFoundation
// CFStream SSL constants are needed only for the legacy CFStream TLS path (when GCDAsyncSocketUseCFStreamForTLS is set)
// The default path uses SecureTransport which doesn't require these constants
// Declare SSL constants as extern to avoid importing deprecated CFNetwork framework
// These symbols are linked at runtime from CFNetwork framework
...
- (instancetype)initWithTLSSettings:(NSDictionary <NSString*,
                                     NSObject*>*)settings NS_DESIGNATED_INITIALIZER;
@end
...
- (BOOL)acceptOnInterface:(NSString *)inInterface port:(uint16_t)port error:(NSError **)errPtr
{
  ...
  int(^createSocket)(int, NSData*) = ^int (int domain, NSData *interfaceAddr) {
    ...
    status = bind(socketFD,
                  (const struct sockaddr *)[interfaceAddr bytes],
                  (socklen_t)[interfaceAddr length]);
    if (status == -1)
    {
...
- (BOOL)acceptOnUrl:(NSURL *)url error:(NSError **)errPtr
{
  ...
  dispatch_block_t block = ^{ @autoreleasepool {
    ...
    dispatch_source_set_event_handler(self->acceptUNSource,
 ^{ @autoreleasepool {
...
- (BOOL)bindSocket:(int)socketFD toInterface:(NSData *)connectInterface error:(NSError **)errPtr
{
  ...
  if (connectInterface)
  {
    ...
    int result = bind(socketFD,
                      interfaceAddr,
                      (socklen_t)[connectInterface length]);
    if (result != 0)
    {
      if (errPtr)
...
- (BOOL)connectWithAddress4:(NSData *)address4 address6:(NSData *)address6 error:(NSError **)errPtr
{
  ...
  if (alternateAddress)
  {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(alternateAddressDelay * NSEC_PER_SEC)),
 socketQueue,
                   ^{
...
- (void)startConnectTimeout:(NSTimeInterval)timeout
{
  if (timeout >= 0.0)
  {
    connectTimer = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER,
                                          0,
                                          0,
                                          socketQueue);
      ...
      while (cursor != NULL)
      {
        if ((addr4 == nil) && (cursor->ifa_addr->sa_family == AF_INET))
        {
          ...
          else
          {
            ...
            const char *conversion = inet_ntop(AF_INET,
        ...
        else if ((addr6 == nil) && (cursor->ifa_addr->sa_family == AF_INET6))
        {
          ...
          else
          {
            ...
            const char *conversion = inet_ntop(AF_INET6,
...
- (NSData *)getInterfaceAddressFromUrl:(NSURL *)url
{
  ...
  strlcpy(nativeAddr.sun_path,
          path.fileSystemRepresentation,
          sizeof(nativeAddr.sun_path));
...
#pragma clang diagnostic warning "-Wimplicit-retain-self"
    ...
    LogVerbose(@"socketFDBytesAvailable: %lu",
               strongSelf->socketFDBytesAvailable);
...
- (void)flushSSLBuffers
{
  ...
  if ([self usingCFStreamForTLS])
  {
    if ((flags & kSecureSocketHasBytesAvailable) && CFReadStreamHasBytesAvailable(readStream))
    {
      ...
      LogVerbose(@"%@ - CFReadStreamRead(): result = %i",
                 THIS_METHOD,
                 (int)result);
  ...
  if (estimatedBytesAvailable > 0)
  {
    ...
    do
    {
      LogVerbose(@"%@ - estimatedBytesAvailable = %lu",
                 THIS_METHOD,
      ...
      OSStatus result = SSLRead(sslContext,
                                buffer,
                                (size_t)estimatedBytesAvailable,
                                &bytesRead);
      LogVerbose(@"%@ - read from secure socket = %u",
                 THIS_METHOD,
                 (unsigned)bytesRead);
      ...
      LogVerbose(@"%@ - prebuffer.length = %zu",
                 THIS_METHOD,
                 [preBuffer availableBytes]);
...
- (void)doReadData
{
  ...
  if ([preBuffer availableBytes] > 0)
  {
    ...
    LogVerbose(@"copied(%lu) preBufferLength(%zu)",
  ...
  if (!done && !error && !socketEOF && hasBytesAvailable)
  {
    ...
    if (flags & kSocketSecure)
    {
      if ([self usingCFStreamForTLS])
      {
        ...
        CFIndex result = CFReadStreamRead(readStream,
                                          buffer,
                                          (CFIndex)bytesToRead);
    ...
    if (bytesRead > 0)
    {
      ...
      else if (currentRead->term != nil)
      {
        ...
        if (readIntoPreBuffer)
        {
          ...
          [preBuffer didWrite:bytesRead];
          LogVerbose(@"read data into preBuffer - preBuffer.length = %zu",
                     [preBuffer availableBytes]);

          // Search for the terminating sequence

          NSUInteger bytesToCopy = [currentRead readLengthForTermWithPreBuffer:preBuffer found:&done];
          LogVerbose(@"copying %lu bytes from preBuffer",
                     (unsigned long)bytesToCopy);
        ...
        else
        {
          ...
          else if (overflow > 0)
          {
            ...
            LogVerbose(@"copying %ld overflow bytes into preBuffer",
                       (long)overflow);
...
- (void)setupReadTimerWithTimeout:(NSTimeInterval)timeout
{
  if (timeout >= 0.0)
  {
    readTimer = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER,
                                       0,
                                       0,
                                       socketQueue);
...
- (void)doReadTimeoutWithExtension:(NSTimeInterval)timeoutExtension
{
  if (currentRead)
  {
    if (timeoutExtension > 0.0)
    {
      ...
      dispatch_time_t tt = dispatch_time(DISPATCH_TIME_NOW,
                                         (int64_t)(timeoutExtension * NSEC_PER_SEC));
      dispatch_source_set_timer(readTimer, tt, DISPATCH_TIME_FOREVER, 0);
...
- (void)doWriteData
{
  ...
  if (flags & kSocketSecure)
  {
    if ([self usingCFStreamForTLS])
    {
      ...
      CFIndex result = CFWriteStreamWrite(writeStream,
                                          buffer,
                                          (CFIndex)bytesToWrite);
      LogVerbose(@"CFWriteStreamWrite(%lu) = %li",
  ...
  if (bytesWritten > 0)
  {
    ...
    LogVerbose(@"currentWrite->bytesDone = %lu",
               (unsigned long)currentWrite->bytesDone);
...
- (void)setupWriteTimerWithTimeout:(NSTimeInterval)timeout
{
  if (timeout >= 0.0)
  {
    writeTimer = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER,
                                        0,
                                        0,
                                        socketQueue);
...
{
  LogVerbose(@"sslReadWithBuffer:%p length:%lu",
             buffer,
  ...
  if (sslPreBufferLength > 0)
  {
    ...
    LogVerbose(@"%@: Copying %zu bytes from sslPreBuffer",
               THIS_METHOD,
               bytesToCopy);
    ...
    LogVerbose(@"%@: sslPreBuffer.length = %zu",
               THIS_METHOD,
  ...
  if (!done && (socketFDBytesAvailable > 0))
  {
    ...
    else
    {
      ...
      if (readIntoPreBuffer)
      {
        ...
        LogVerbose(@"%@: Copying %zu bytes out of sslPreBuffer",
                   THIS_METHOD,
                   bytesToCopy);

        memcpy((uint8_t *)buffer + totalBytesRead,
               [sslPreBuffer readBuffer],
               bytesToCopy);
        [sslPreBuffer didRead:bytesToCopy];
...
static OSStatus SSLReadFunction(SSLConnectionRef connection,
                                void *data,
                                size_t *dataLength)
{
  GCDAsyncSocket *asyncSocket = (__bridge GCDAsyncSocket *)connection;

  NSCAssert(dispatch_get_specific(asyncSocket->IsOnSocketQueueOrTargetQueueKey),
            @"What the deuce?");

  return [asyncSocket sslReadWithBuffer:data length:dataLength];
}

static OSStatus SSLWriteFunction(SSLConnectionRef connection,
                                 const void *data,
                                 size_t *dataLength)
{
  GCDAsyncSocket *asyncSocket = (__bridge GCDAsyncSocket *)connection;
...
- (void)ssl_startTLS
{
  ...
  {
    if (isServer)
      sslContext = SSLCreateContext(kCFAllocatorDefault,
                                    kSSLServerSide,
                                    kSSLStreamType);
    else
      sslContext = SSLCreateContext(kCFAllocatorDefault,
  ...
  if ([shouldManuallyEvaluateTrust boolValue])
  {
    ...
    status = SSLSetSessionOption(sslContext,
...
#pragma clang diagnostic pop
  ...
  if ([value isKindOfClass:[NSData class]])
  {
    ...
    status = SSLSetDiffieHellmanParams(sslContext,
                                       [diffieHellmanData bytes],
                                       [diffieHellmanData length]);
    if (status != noErr)
    {
  ...
  {
    NSAssert(NO,
             @"Invalid value for GCDAsyncSocketSSLDiffieHellmanParameters. Value must be of type NSData.");
  ...
  {
    NSAssert(NO,
             @"Invalid value for GCDAsyncSocketSSLALPN. Value must be of type NSArray.");
...
- (void)ssl_shouldTrustPeer:(BOOL)shouldTrust stateIndex:(int)aStateIndex
{
  ...
  if (shouldTrust)
  {
    NSAssert(lastSSLHandshakeError == errSSLPeerAuthCompleted,
...
- (void)cf_startTLS
{
  ...
  NSAssert([currentRead isKindOfClass:[GCDAsyncSpecialPacket class]],
           @"Invalid read packet for startTLS");
  NSAssert([currentWrite isKindOfClass:[GCDAsyncSpecialPacket class]],
  ...
  // CFNetwork/CFStream.h is imported for SSL constants needed by CFStream TLS support.

  BOOL r1 = CFReadStreamSetProperty(readStream,
                                    kCFStreamPropertySSLSettings,
                                    tlsSettings);
  BOOL r2 = CFWriteStreamSetProperty(writeStream,
                                     kCFStreamPropertySSLSettings,
                                     tlsSettings);
...
+ (void)startCFStreamThreadIfNeeded
{
 ...
 ^{
    ...
    cfstreamThreadSetupQueue = dispatch_queue_create("GCDAsyncSocket-CFStreamThreadSetup",
...
{
  LogTrace();
  NSAssert([NSThread currentThread] == cfstreamThread,
           @"Invoked on wrong thread");

  CFRunLoopRef runLoop = CFRunLoopGetCurrent();

  if (asyncSocket->readStream)
    CFReadStreamScheduleWithRunLoop(asyncSocket->readStream,
                                    runLoop,
                                    kCFRunLoopDefaultMode);

  if (asyncSocket->writeStream)
    CFWriteStreamScheduleWithRunLoop(asyncSocket->writeStream,
                                     runLoop,
                                     kCFRunLoopDefaultMode);
}
...
+ (void)unscheduleCFStreams:(GCDAsyncSocket *)asyncSocket
{
  ...
  if (asyncSocket->readStream)
    CFReadStreamUnscheduleFromRunLoop(asyncSocket->readStream,
                                      runLoop,
                                      kCFRunLoopDefaultMode);

  if (asyncSocket->writeStream)
    CFWriteStreamUnscheduleFromRunLoop(asyncSocket->writeStream,
                                       runLoop,
                                       kCFRunLoopDefaultMode);
}
...
- (BOOL)registerForStreamCallbacksIncludingReadWrite:(BOOL)includeReadWrite
{
  ...
  NSAssert(dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey),
           @"Must be dispatched on socketQueue");
  NSAssert((readStream != NULL && writeStream != NULL),
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
  r1 = CFReadStreamSetProperty(readStream,
                               kCFStreamNetworkServiceType,
                               kCFStreamNetworkServiceTypeVoIP);
  r2 = CFWriteStreamSetProperty(writeStream,
                                kCFStreamNetworkServiceType,
                                kCFStreamNetworkServiceTypeVoIP);
#pragma clang diagnostic pop
