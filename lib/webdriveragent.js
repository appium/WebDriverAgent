...
class WebDriverAgent {
  ...
  async _cleanupProjectIfFresh () {
    ...
    if (await fs.exists(timestampPath)) {
      ...
      if (_.isInteger(recentUpgradeTimestamp)) {
        if (recentUpgradeTimestamp >= currentUpgradeTimestamp) {
          log.info(`WDA does not need a cleanup. The sources are up to date ` +
            `(${recentUpgradeTimestamp} >= ${currentUpgradeTimestamp})`);
          return;
        }
        log.info(`WDA sources have been upgraded (${recentUpgradeTimestamp} < ${currentUpgradeTimestamp})`);
      } else {
        log.warn(`The recent upgrade timestamp at '${timestampPath}' is corrupted. Trying to fix it`);
        await fs.rimraf(timestampPath);
      }
    }
    ...
    try {
      ...
      log.debug(`Set the recent WDA upgrade timestamp to ${currentUpgradeTimestamp} at '${timestampPath}'`);
    } catch (e) {
      log.warn(`Unable to create the recent WDA upgrade timestamp at '${timestampPath}'. ` +
        `Original error: ${e.message}`);
      return;
    }
    ...
    await SHARED_RESOURCES_GUARD.acquire(synchronizationKey,
      async () => await this.xcodebuild.cleanProject());
  }
  ...
  async launch (sessionId) {
    ...
    if (this.idb || this.useXctestrunFile || (this.derivedDataPath && this.usePrebuiltWDA)) {
      ...
      await this._cleanupProjectIfFresh();
    }
