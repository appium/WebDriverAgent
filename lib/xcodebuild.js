...
export class XcodeBuild {
  ...
  getCommand (buildOnly = false) {
    ...
    if (this.useSimpleBuildTest) {
      this.log.info("'appium:useSimpleBuildTest' is deprecated, which was to build the WebDriverAgent project with 'build' and " +
        "'test' legacy commands for older than Xcode 8. Please remove the capability.");
    }
